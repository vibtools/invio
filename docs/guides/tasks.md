...
2. one or more available Accounts for that provider;
3. Invoice Template;
4. Customer List with at least one email.

The selected template ID/name is stored on the task. A template assigned to an open task cannot be deleted. Selected accounts are reserved and cannot be selected by another task.

## Controls

Existing task controls remain: **Start**, **Pause**, **Resume**, **Stop**, **Retry Failed**, and **Close Task**.

- Start obtains the built-in runner for packaged providers or an explicitly registered custom runner.
- Pause/Resume/Stop act on that task's own worker slot.
- Retry Failed uses provider runtime failure state to retry failed Stripe recipients only.
