...
USER_AGENT = "Invio-ZohoBooks-Provider/1.2.0 Vib-Tools"

# Zoho Books v3 publishes these eight API data-center domains.  The Accounts
# host is supplied explicitly by the user so the adapter never guesses a DC.
_ACCOUNTS_TO_BOOKS_API = {
...
class Adapter:
    ...
    # Zoho Books documents 100 API calls/minute per organization.  Invio's
    ...
    browser_oauth_profile = BrowserOAuthProfile(
        button_label="Connect Zoho Books",
        redirect_uri="http://127.0.0.1:8765/oauth/callback/zoho-books",
        pkce_required=True,
    ...
    def _oauth_accounts_server(value: str) -> tuple[str, str]:
        ...
        if (
            ...
            or parsed.query
            or parsed.fragment
            or host not in _ACCOUNTS_TO_BOOKS_API
        ):
    ...
    def build_oauth_authorization_url(self, context) -> str:
        ...
        if not client_id:
            raise ProviderRuntimeError("OAuth Client ID is required before connecting Zoho Books.", category="preflight")
        params = {
            "scope": "ZohoBooks.settings.READ,ZohoBooks.settings.CREATE,ZohoBooks.contacts.READ,ZohoBooks.contacts.CREATE,ZohoBooks.invoices.READ,ZohoBooks.invoices.CREATE",
            "client_id": client_id,
            "response_type": "code",
    ...
    def complete_oauth_authorization(self, context) -> ExternalOAuthConnectionResult:
        ...
        if not client_id or not client_secret:
            raise ProviderRuntimeError("OAuth Client ID and Client Secret are required to complete Zoho Books authorization.", category="preflight")
        ...
        api_origin = _ACCOUNTS_TO_BOOKS_API[host]
        organizations = context.request(
            stage="oauth_account_discovery",
            method="GET",
            url=f"{api_origin}/books/v3/organizations",
        ...
        if not isinstance(organizations, dict):
            raise ProviderRuntimeError("Zoho Books organization discovery returned an unexpected response.", category="response")
        rows = organizations.get("organizations")
        if not isinstance(rows, list):
            raise ProviderRuntimeError("Zoho Books organization discovery did not return an organizations list.", category="response")
        choices = []
        for row in rows:
            ...
            if not organization_id:
                continue
            label = str(row.get("name", "")).strip() or f"Zoho Books {organization_id}"
            choices.append(ExternalOAuthAccountChoice(organization_id, label))
        if not choices:
            raise ProviderRuntimeError("No Zoho Books organization is available to the authorized user.", category="preflight", retryable=False)
        return ExternalOAuthConnectionResult(
            credential_updates={"accounts_server_url": accounts_server, "refresh_token": refresh_token},
            message="Zoho Books authorization completed. Invio will refresh access tokens automatically.",
    ...
    def prepare_account(self, context) -> ExternalOnboardingResult:
        ...
        access_token, books_api = self._access_token(context, account)
        organization_id = account[4]
        organization = self._books_request(
            context,
        ...
        if not isinstance(org, dict) or str(org.get("organization_id", "")).strip() != organization_id:
            raise ProviderRuntimeError("Zoho Books organization could not be verified during Easy Onboarding.", category="response")
        account_label = str(org.get("name", "")).strip() or "Zoho Books"

        default_item_id = account[5]
        if default_item_id:
            item = self._books_request(
                context, access_token=access_token, books_api=books_api, organization_id=organization_id,
            ...
            if not isinstance(item, dict) or self._positive_numeric_id(item.get("item_id")) != default_item_id:
                raise ProviderRuntimeError("Configured Zoho Books Default Item ID could not be verified.", category="preflight")
            if str(item.get("status", "active")).strip().lower() not in {"", "active"}:
                raise ProviderRuntimeError("Configured Zoho Books Default Item ID is inactive.", category="preflight")
            managed_item_id = default_item_id
            created = False
        else:
            result = self._books_request(
                context, access_token=access_token, books_api=books_api, organization_id=organization_id,
            ...
            if not isinstance(items, list):
                raise ProviderRuntimeError("Zoho Books items response is missing items list.", category="response")
            matches = []
            for item in items:
            ...
            if len(unique) > 1:
                raise ProviderRuntimeError("Multiple active Zoho Books items are named 'Invio Service'; managed resource selection is ambiguous.", category="preflight")
            if unique:
                managed_item_id = unique[0]
                created = False
            else:
                created_result = self._books_request(
                    context, access_token=access_token, books_api=books_api, organization_id=organization_id,
                ...
                if not managed_item_id or not isinstance(item, dict) or str(item.get("name", "")).strip() != "Invio Service":
                    raise ProviderRuntimeError("Zoho Books managed item creation did not return the expected Invio Service item.", category="response")
                created = True

        action = "created" if created else "reused"
        return ExternalOnboardingResult(
            credential_updates={"default_item_id": managed_item_id},
            message=f"Zoho Books account prepared. Invio Service {action} and provider-managed credentials are ready.",
    ...
    def test_account(self, context):
        ...
        if not isinstance(org, dict) or str(org.get("organization_id", "")).strip() != account[4]:
            raise ProviderRuntimeError(
                "Zoho Books organization verification did not return the configured organization ID.",
        ...
        default_item_id = account[5]
        if default_item_id:
            item_result = self._books_request(
                context,
                access_token=access_token,
                books_api=books_api,
                organization_id=account[4],
            ...
            if not isinstance(item, dict) or str(item.get("item_id", "")).strip() != default_item_id:
                raise ProviderRuntimeError(
                    "Zoho Books Default Item ID did not resolve to the configured item.",
                    category="provider-contract",
                    retryable=False,
                )
            if str(item.get("status", "active")).strip().lower() not in {"", "active"}:
                raise ProviderRuntimeError(
                    "Zoho Books Default Item ID is not active.",
                    category="provider-state",
                    retryable=False,
                )

        return "Zoho Books API connection verified."
    ...
    def validate_task(self, context):
        ...
        if str(template.invoice_type).strip().upper() != "INVOICE":
            issues.append(
                ExternalValidationIssue(
                    "zoho_books_invoice_type",
                    "Zoho Books provider supports customer invoices only.",
        ...
        if str(template.invoice_title).strip() not in {"", "Invoice"}:
            issues.append(
                ExternalValidationIssue(
                    "zoho_books_invoice_title",
                    "Zoho Books provider does not map a custom Invio invoice title.",
        ...
        if str(template.invoice_subtitle).strip():
            issues.append(
                ExternalValidationIssue(
                    "zoho_books_invoice_subtitle",
                    "Zoho Books provider does not map the Invio invoice subtitle field.",
        ...
        if bool(template.automatic_tax):
            issues.append(
                ExternalValidationIssue(
                    "zoho_books_automatic_tax",
                    "Zoho Books provider does not infer automatic tax configuration.",
        ...
        if str(template.footer).strip():
            issues.append(
                ExternalValidationIssue(
                    "zoho_books_footer",
                    "Zoho Books provider does not map the Invio footer field.",
        ...
        if str(template.customer_note).strip():
            issues.append(
                ExternalValidationIssue(
                    "zoho_books_customer_note",
                    "Zoho Books provider does not map the Invio customer-note field.",
        ...
        if not template.items:
            issues.append(
                ExternalValidationIssue(
                    "zoho_books_items",
                    "Zoho Books invoice requires at least one line item.",
        ...
        for index, item in enumerate(template.items, start=1):
            ...
            if not description:
                issues.append(
                    ExternalValidationIssue(
                        "zoho_books_item_name",
                        f"Zoho Books invoice item {index} requires a non-empty description/item name.",
                        "Set the Invio item description to an existing Zoho Books Item name or configure Default Item ID.",
                    )
                )
            if Decimal(item.quantity) <= 0:
                issues.append(
                    ExternalValidationIssue(
                        "zoho_books_quantity",
                        f"Zoho Books invoice item {index} must have a positive quantity.",
            ...
            if Decimal(item.unit_amount) < 0:
                issues.append(
                    ExternalValidationIssue(
                        "zoho_books_unit_amount",
                        f"Zoho Books invoice item {index} cannot use a negative unit amount in this adapter.",
            ...
            if Decimal(item.tax_rate) != 0:
                issues.append(
                    ExternalValidationIssue(
                        "zoho_books_line_tax",
                        f"Zoho Books invoice item {index} has a non-zero Invio line-tax rate that cannot be mapped safely without a Zoho tax_id.",
    ...
    def execute_recipient(self, context):
        ...
        invoice_result = self._books_request(
            context,
            access_token=access_token,
            books_api=books_api,
            organization_id=organization_id,
        ...
        if not invoice_id:
            raise ProviderRuntimeError(
                "Zoho Books invoice creation succeeded without a usable invoice_id in the response.",
                category="response",
                retryable=False,
            )
        context.log(f"Zoho Books invoice created (provider invoice {invoice_id}).")

        send_result = self._books_request(
            context,
            access_token=access_token,
            books_api=books_api,
            organization_id=organization_id,
        ...
        if message:
            context.log(f"Zoho Books accepted invoice email request: {message}")
        else:
            context.log("Zoho Books accepted invoice email request.")
    ...
    def _account_values(credentials: dict[str, str]) -> tuple[str, str, str, str, str, str]:
        ...
        missing = [
            ...
            for label, value in (
                ...
                ("Zoho Books Organization ID", organization_id),
            )
            if not value
        ]
        if missing:
            raise ProviderRuntimeError(
                "Zoho Books account is missing required credential field(s): " + ", ".join(missing) + ".",
                category="preflight",
                retryable=False,
            )
        ...
        if not organization_id.isdigit():
            raise ProviderRuntimeError(
                "Zoho Books Organization ID must contain numeric characters only.",
                category="preflight",
                retryable=False,
            )
        if default_item_id and not default_item_id.isdigit():
            raise ProviderRuntimeError(
                "Zoho Books Default Item ID must contain numeric characters only when provided.",
                category="preflight",
                retryable=False,
            )
    ...
    def _access_token(self, context, account: tuple[str, str, str, str, str, str]) -> tuple[str, str]:
        ...
        books_api = _ACCOUNTS_TO_BOOKS_API[accounts_host]
        try:
        ...
        with self._cache_lock:
            self._token_cache[cache_key] = (access_token, books_api, time.monotonic() + usable_for)
        return access_token, books_api

    # ------------------------------------------------------------------
    # Zoho Books REST helpers
    # ------------------------------------------------------------------
    ...
    def _books_request(
        ...
        *,
        ...
        if clean_path != f"/organizations/{organization_id}":
            query_data.setdefault("organization_id", organization_id)
        query_string = urlencode(query_data, doseq=True)
        url = f"{books_api}/books/v3{clean_path}"
        if query_string:
            url += "?" + query_string
        response = context.request(
            stage=stage,
        ...
        if not isinstance(response, dict):
            raise ProviderRuntimeError("Zoho Books response was not a JSON object.", category="response")
        if "code" in response:
            ...
            try:
                ...
                numeric_code = -1
            if numeric_code != 0:
                message = str(response.get("message", "Zoho Books API returned a non-success response.")).strip()
                raise ProviderRuntimeError(
                    f"Zoho Books API error code {code!s}: {message}",
                    category="provider",
                    retryable=False,
                )
        ...
        *,
        ...
        while page <= 20:
            ...
            if not isinstance(currencies, list):
                raise ProviderRuntimeError("Zoho Books currencies response is missing currencies list.", category="response")
            ...
            page += 1
        raise ProviderRuntimeError(
            f"Zoho Books organization does not expose configured currency {wanted}.",
            category="preflight",
            retryable=False,
        )
    ...
    def _resolve_item(
        ...
        *,
        ...
        if default_item_id:
            ...
            if not item_id:
                raise ProviderRuntimeError("Zoho Books Default Item ID could not be resolved.", category="preflight")
            if str(item.get("status", "active")).strip().lower() not in {"", "active"}:
                raise ProviderRuntimeError("Zoho Books Default Item ID is inactive.", category="preflight")
            with self._cache_lock:
        ...
        if len(unique) != 1:
            if not unique:
                raise ProviderRuntimeError(
                    f"No active Zoho Books Item exactly matches Invio line-item description '{wanted}'. "
                    "Create that Zoho Item or configure Default Item ID on the Invio account.",
                    category="preflight",
                    retryable=False,
                )
            raise ProviderRuntimeError(
                f"Multiple active Zoho Books Items exactly match '{wanted}'; item mapping is ambiguous.",
                category="preflight",
                retryable=False,
            )
        with self._cache_lock:
        ...
        *,
        ...
        if bool(context.template.reuse_customer):
            ...
            if not isinstance(contacts, list):
                raise ProviderRuntimeError("Zoho Books contacts response is missing contacts list.", category="response")
            ...
            if len(exact) > 1:
                raise ProviderRuntimeError(
                    f"Multiple active Zoho Books customer contacts use {email}; customer reuse is ambiguous.",
                    category="preflight",
                    retryable=False,
                )
            if exact:
                ...
                if not contact_id:
                    raise ProviderRuntimeError("Zoho Books reused contact is missing contact_id.", category="response")
                ...
                if existing_currency_id and existing_currency_id != currency_id:
                    raise ProviderRuntimeError(
                        f"Zoho Books customer {email} is assigned to currency {existing_currency_code or existing_currency_id}, "
                ...
                if existing_currency_code and existing_currency_code != currency_code:
                    raise ProviderRuntimeError(
                        f"Zoho Books customer {email} uses {existing_currency_code}, but the Invio template uses {currency_code}.",
                        category="preflight",
                        retryable=False,
                    )
                context.log(f"Zoho Books customer found for {email} (contact {contact_id}).")
                return contact_id
        ...
        if not contact_id:
            raise ProviderRuntimeError(
                "Zoho Books contact creation succeeded without a usable contact_id in the response.",
                category="response",
                retryable=False,
            )
        context.log(f"Zoho Books customer created for {email} (contact {contact_id}).")
        return contact_id
