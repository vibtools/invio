...
class BrowserOAuthTests(unittest.TestCase):
    def test_browser_oauth_pkce_exchange_and_org_discovery(self):
        ...
        parsed = urlsplit(adapter.build_oauth_authorization_url(auth))
        query = parse_qs(parsed.query)
        self.assertEqual(parsed.netloc, "accounts.zoho.com")
        self.assertEqual(query["access_type"], ["offline"])
        self.assertEqual(query["code_challenge_method"], ["S256"])
        calls=[]
        def request(**kwargs):
            calls.append(kwargs)
            if kwargs["url"].endswith("/oauth/v2/token"):
                return {"access_token":"ACCESS","refresh_token":"REFRESH"}
            if kwargs["url"].endswith("/books/v3/organizations"):
                return {"organizations":[{"organization_id":"123","name":"Vib Tools"}]}
        ...
        self.assertEqual(result.choices[0].value,"123")
        self.assertNotIn("access_token",result.credential_updates)
