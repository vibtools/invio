...
class _RequestRecorder:
    ...
    def __call__(self, **kwargs):
        ...
        if "/books/v3/organizations/" in parsed.path:
            return {"code": 0, "message": "success", "organization": {"organization_id": "10234695"}}
        if parsed.path.endswith("/books/v3/settings/currencies"):
            return {
                "code": 0,
                "message": "success",
        ...
        if parsed.path.endswith("/books/v3/items") and method == "GET":
        ...
        if "/books/v3/items/" in parsed.path:
            return {"code": 0, "message": "success", "item": {"item_id": parsed.path.rsplit("/", 1)[-1], "status": "active"}}
        if parsed.path.endswith("/books/v3/contacts") and method == "GET":
        ...
        if parsed.path.endswith("/books/v3/contacts") and method == "POST":
            return {"code": 0, "message": "created", "contact": {"contact_id": "6002"}}
        if parsed.path.endswith("/books/v3/invoices") and method == "GET":
            return {"code": 0, "message": "success", "invoices": []}
        if parsed.path.endswith("/books/v3/invoices") and method == "POST":
            return {"code": 0, "message": "created", "invoice": {"invoice_id": "8001"}}
        if parsed.path.endswith("/books/v3/invoices/8001/email") and method == "POST":
...
class ZohoBooksProviderTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        spec = importlib.util.spec_from_file_location("invio_zoho_books_test_adapter", BUNDLE / "adapter.py")
    ...
    def test_manifest_and_adapter_identity_match(self):
        ...
        self.assertEqual(manifest["id"], "zoho_books")
    ...
    def test_existing_customer_invoice_create_and_real_email_endpoint(self):
        ...
        invoice_calls = [c for c in recorder.calls if urlsplit(c["url"]).path.endswith("/books/v3/invoices") and c["method"] == "POST"]
        ...
        send_calls = [c for c in recorder.calls if urlsplit(c["url"]).path.endswith("/books/v3/invoices/8001/email")]
    ...
    def test_validation_fails_closed_for_unmappable_fields(self):
        ...
        self.assertIn("zoho_books_automatic_tax", codes)
        self.assertIn("zoho_books_footer", codes)
        self.assertIn("zoho_books_customer_note", codes)
        self.assertIn("zoho_books_line_tax", codes)
...
class ZohoBooksInvioP13IntegrationTests(unittest.TestCase):
    def test_bundle_installs_and_executes_through_real_invio_external_runtime_contract(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(provider.id, 'zoho_books')
            self.assertEqual(runtime.runtime_capabilities('zoho_books'), ('invoice', 'send_invoice', 'api_test'))
            ...
            account = state.add_account(
                'zoho_books', 'Zoho Books', 'Zoho Main', 'Default',
                {
                    'accounts_server_url': 'https://accounts.zoho.com',
                    'client_id': '1000.CLIENT',
                    'client_secret': 'SECRET',
            ...
            task = state.create_task('zoho_books', 'Zoho Books', [account.id], customer_list.id, template.id)
            ctx = RunContext(task)
            ...
            self.assertTrue(any('/books/v3/invoices/8001/email' in url for _m, url, _h, _b in calls))
