...
class RepositoryContractTests(unittest.TestCase):
    def test_release_metadata_is_v100141(self):
        ...
        self.assertIn('version = "1.0.0.1.50"', pyproject)
        self.assertIn('"version": "1.0.0.1.50"', project_meta)
        self.assertIn('"latestVersion": "1.0.0.1.50"', project_meta)
        self.assertIn('"current": "1.0.0.1.50"', docs_meta)
        self.assertIn('"latest": "1.0.0.1.50"', docs_meta)
        self.assertIn('Production • v1.0.0.1.50', main_window)
        self.assertIn('Invio/1.0.0.1.50', runtime)
    ...
    def test_v100136_ci_correction_and_certification_truthfulness(self):
        ...
        self.assertIn('INVIO_VERSION: "1.0.0.1.50"', workflow)
        self.assertIn('INVIO_PE_VERSION: "1.0.1.50"', workflow)
        self.assertIn("P14 CERTIFICATION PENDING", roadmap)
        self.assertIn("Production-ready: NO", release)
...
class V150DynamicTagsReleaseContractTests(unittest.TestCase):
    def test_v150_identity_dynamic_tags_schema_and_frozen_boundaries_are_truthful(self):
        ...
        if project_root.is_dir():
            ...
            self.assertIn('This candidate contains only', freeze)
            self.assertIn('deterministic R5/R11', verification)
